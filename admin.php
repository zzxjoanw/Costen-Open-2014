<?
    ...
    $conn = new mysqli($host, $username, $password, $dbname) or die("Unable to connect to database. Please try again later.");
        ...
        <nav class="navbar navbar-default">
            ...
            <?
                if($_SESSION['isLoggedIn'] == 0)
                {
                    ...
                    {
                        if(isset($_POST['bttnSubmit'])) { echo "bad username and/or password"; }
                    ?>
                ...
                {   
                    if($_GET['action'] == "view")
                    {
                        if(!$pps = $conn->prepare("SELECT * FROM tblContestant c, tblParticipation p WHERE c.AGANumber = ? AND
                            p.AGANumber = c.AGANumber"))
                        {
                            echo "Error: " . $conn->error;
                        }
                        
                        if(!$pps->bind_param('s',$_GET['num']))
                        {
                            echo "Error: " . $conn->error;
                        }
                        ...
                        while($pps->fetch())
                        {
                            ?>
                                <form action="admin.php?action=update&num=<? echo $AGANumber ?>" method="post">
                                    ...
                                    <input type="text" value="<? echo $lname ?>" name="lname">
                                    ...
                                    <label for="club">Last (Family) Name:</label>
                    ...
                    {
                        echo "<form action='admin.php?action=confirm&AGANumber=" . $_GET['num'] . "' method='post'>";
                    ...
                    else if($_GET['action'] == 'confirm')
                    {
                        ...
                        if(!$pps = $conn->prepare("DELETE FROM tblContestant WHERE AGANumber = ?"))
                        {
                            echo "Error: " . $pps->error;
                        }
                        
                        if(!$pps->bind_param("s",$_GET['AGANumber']))
                        {
                            echo "Error: " . $pps->error;
                        }
                    ...
                    {
                        $pps = $conn->prepare("SELECT fName,lName,AGANumber,club FROM tblContestant");
                        $pps->execute();
                        $pps->bind_result($fname,$lname,$AGANumber,$club);
                        ...
                        while($row = $pps->fetch())
                        {
                            ...
                            echo "<td>";
                            echo "<a href=\"admin.php?action=view&num=".$AGANumber."\">";
                            echo "<span class=\"history glyphicon glyphicon-zoom-in\"></span>";
                            echo "</a>";
                            echo "</td>";
                            
                            echo "<td>";
                            echo "<a href=\"admin.php?action=delete&num=".$AGANumber."\">";
